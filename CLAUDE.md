...
7. **Continuous QA feeds back into tasks — but only findings that can fail.** QA runs *during* the build, not only at the end, and a finding that comes with a **reproducing failure** (a test that reds, a defect a user hits, a security hole) becomes a task file, not a lost note. A finding that cannot produce one — "these blocks are similar", "this could drift", "a future module might" — is recorded **where it would bite** (a comment at each site naming the source of truth) and is **not filed**. The task list is work, not a worry log. This clause is itself a finding: by 2026-09 the last 35 tasks were ~100% process-about-process and five said, in their own text, that nothing currently fails — because §2.7 as originally written made *every* confirmed observation a task, and reviews of those tasks produced more. A drift-prevention task with no failing input usually costs more to carry than the drift would cost to fix. **Corollary: do not build a gate whose only job is watching another gate** — prefer fewer, louder gates that run the real thing against reality (a device, a real database, the real engine) over a supervisory tier that re-derives a verdict someone upstream already had (D25, task 194).
...
| UI strings, locales, label keys | `07-i18n.md` |

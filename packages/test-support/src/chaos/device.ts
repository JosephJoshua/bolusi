...
//
// PLATFORM/DOMAIN-FREE (task 181): the DB connection and the notes module are supplied through
// `ConvergenceSeams` (seams.ts), never imported here — so this file bundles on Hermes (the on-device
// rig) with no better-sqlite3, no `@bolusi/modules`, no `node:` edge. The Node harness and the
// on-device env each bind their own seams and share this ONE device driver (§2.8).
import {
  createModuleRuntime,
...
import type { FakeClock } from '../determinism/clock.js';
import { makeIdSource } from '../determinism/id-source.js';
import type { Prng } from '../determinism/prng.js';

import { insertPulledOp, readWireOps, type ClientDbHandle } from './client-db.js';
...
/**
 * A module to register on a device BEYOND the seam's primary module (which is always registered),
 * paired with its permission manifest for the grant-all evaluator. CHAOS-07 adds `platform` so a
 * device can fold the server-minted `platform.conflict_detected` op into its `conflicts` projection
 * and author `platform.conflict_acknowledged` through the real `acknowledgeConflict` command
 * (04 §5.1 — the only production write path). Every other scenario is notes-only, so this defaults
 * to empty.
 */
...
export class VirtualDevice {
  ...
  private constructor(
    ...
    /** The primary module's projection manifest (seam) — the digest input (§3.4). */
    private readonly projectionManifest: ModuleProjectionManifest<ClientDatabase>,
  ) {}
  ...
  /**
   * Wire a device: real client DB + migrations (via `seams.openDb`), the real engine (with a stats
   * sink), the real command/query runtimes over a grant-all evaluator, then the genesis enrollment op
   * so the device can command (05 §9.5). `prng` seeds the id source; `clock` is the ONLY time source.
   */
  static async open(
    options: {
      readonly identity: DeviceIdentity;
      readonly clock: FakeClock;
      readonly prng: Prng;
      /** Modules to register beyond the seam's primary (CHAOS-07 adds `platform`). Default: none. */
    ...
    const handle = await seams.openDb();
    ...
    const evaluator = await buildGrantAllEvaluator({
      ...
      manifests: [seams.moduleManifest, ...extraModules.map((m) => m.permissionManifest)],
    });
  ...
  digest(): Promise<string> {
    return digestModule(this.handle.db, this.projectionManifest, {

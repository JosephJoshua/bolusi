...
//
// PLATFORM/DOMAIN-FREE (task 181): the DB connection and the module trio arrive through
// `ConvergenceSeams` (seams.ts), so this file names neither better-sqlite3 nor `@bolusi/modules`.
...
import { insertPulledOp } from './client-db.js';
import type { ConvergenceSeams } from './seams.js';

const hash = (data: Uint8Array): Uint8Array => noblePort.sha256(data);

/**
 * The canonical-fold reference for an op set (§3.4): a fresh DB via `seams.openDb`, all ops fed to
 * the REAL projection engine strictly in `(timestamp ASC, deviceId ASC, seq ASC)` order, then
 * digested. Every op is canonically newest for its entity at insertion, so this is a pure head-apply
 * fold — asserted, so a reference that silently re-folded (a comparator bug) fails rather than
 * masking one.
 */
export async function canonicalFold(
  ...
  try {
    const registry = registerModules<ClientDatabase>([seams.module]);
    ...
    const digest = await digestModule(handle.db, seams.projectionManifest, { hash });

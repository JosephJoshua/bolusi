...
//
// PLATFORM/DOMAIN-FREE (task 181): `seams` carries the DB engine + module trio to `VirtualDevice`
// and `canonicalFold`; this orchestrator names no platform or domain values.
import type { SignedOperation } from '@bolusi/schemas';

import { FakeClock } from '../determinism/clock.js';
import { mulberry32, randomInt, shuffle, type Prng } from '../determinism/prng.js';
...
export async function runConvergence(
  ...
  const reference = await canonicalFold(universe, seams);

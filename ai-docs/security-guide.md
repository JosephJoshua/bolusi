...
**Documented exceptions — exactly TWO in v0, enumerated below.** Each departs from the table in its own direction and on its own argument: exception 1 *refuses* a distinction the table requires (a `404` where row 2 says `403`), exception 2 *keeps* one the table forbids (a `403` that reveals a token is already taken). They do not share a justification — read each on its own, and note what exception 2 says about arguments that rest on a third party's undocumented behaviour. The list is **closed and mechanically pinned** — SEC-TENANT-04 parses this section, requires the harness's exception set to equal it exactly (a third exception documented here with no probe, or probed with no entry here, fails the sweep), and asserts that every *other* endpoint answers a cross-tenant id and a nonexistent one indistinguishably. Do not add a third by editing this paragraph: an exception is an owner ruling (CLAUDE.md §6), recorded in `decisions/` first.

**The default for a newly-found existence oracle is to REMOVE it, not to document it — and the count stays at two.** The one candidate since (task 141a's sweep: `POST /v1/media/:id/init` answered `404` for a media id another tenant held and `200` for a free one) was ruled the other way in **D23 §2** — media id uniqueness becomes `(tenant_id, id)`, so an id another tenant holds simply does not exist in yours and both cases answer `200`. The oracle stops existing rather than joining this list. What decided it was that exception 2's justification could not be borrowed: its only leg is the probe budget, and that route's budget is `perRoutePerMinute: 120` (`apps/server/src/deps.ts:71`) ≈ 172,800 probes/day against UUIDv7's 74 random bits — three orders of magnitude looser than push registration's 30/day, and a rate rather than a daily cap. **A documented exception needs a justification that survives its own numbers; check them at the producer before assuming two routes are comparable.**

**Documented exception 1 — id-keyed resource probes (`GET /v1/media/:id`).** Media download returns `404 MEDIA_NOT_FOUND` for every out-of-scope id: cross-tenant, same-tenant unassigned store, another device's in-flight upload, and nonexistent are indistinguishable (api/03-media §2). Justification: FR-1036's "permission error, never an empty result" governs *navigational* queries, where the caller addresses a scope and a silently-filtered result would lie about it. A blind fetch by resource id is the opposite shape — any response that distinguishes "exists but denied" from "does not exist" (`403` vs `404`) is an existence oracle over ids the caller was never shown. Media ids reach a client only inside ops it was authorized to pull, so the legitimate case never needs the distinction. SEC-MEDIA-03 asserts the `404` on every leg.

**Documented exception 2 — push-token registration (`POST /v1/push/tokens`).** Owner ruling D22 §2 (task 141a). An Expo token already held by a device of **another tenant** returns `403 PERMISSION_DENIED` — fail-closed, because RLS hides the foreign row and ownership cannot transfer across tenants (api/04-push §2; task 118) — while a token nobody holds returns `200`. Held-vs-fresh is therefore distinguishable: a cross-tenant existence check on the token value.

**Justification — the probe budget, which is enforced in code this repo owns.** The 30-per-day-per-device registration limit (api/04-push §2) is charged at `apps/server/src/routes/push.ts:43-50`, **before** the collision path throws at `:100`, so every probe costs one of thirty per device per day whether it is answered `403` or `200`. An oracle that answers thirty questions a day enumerates nothing at any plausible key width, and a device spending its budget probing is counted in the same limiter. Secondarily — and this is a property of the *interaction*, not of the token — a caller must already **present** a value to learn anything about it, so a `403` confirms something it possesses and never yields a value it did not bring.

**What this exception must NOT be argued from: the token's entropy.** Expo publishes no length, character set, randomness, or secrecy property for what `getExpoPushTokenAsync` returns — the documented return is an `ExpoPushToken` object and nothing more (checked against the current `expo-notifications` docs, 2026-07-23). Our own validator constrains only the wrapper (`/^ExponentPushToken\[[^\]]+\]$/`, `apps/server/src/push/schemas.ts`), and Expo's `isExpoPushToken` checks only prefix and suffix. The one artefact in this repo with a countable width is a **test fixture** (`apps/server/test/helpers/push.ts` — 22 hex characters, hence the "~88 bits" that D22 §2 and an earlier draft of this paragraph both repeated). That number describes a double, not the real token; a bit count for a real token would be an assumption about a third party's generator. A security exception may not rest on an unverifiable premise (CLAUDE.md §2.11 / testing-guide T-16 — *a mention is not a producer*), and the ruling does not need it: the budget above is sufficient on its own.

The alternative was weighed and rejected: making the two indistinguishable means either a lying `200` — the device then believes it is registered and silently receives no pushes, a failure no budget bounds — or dropping the fail-closed check and breaching tenant isolation. **The exception covers the token value only.** The other `403` on this route (body `deviceId` ≠ the bearer's device) is not an existence oracle — it answers identically for a foreign device id and a nonexistent one, and SEC-TENANT-04's control leg asserts exactly that. SEC-TENANT-04's exception leg asserts the documented pair (`403` held / `200` fresh), so this paragraph is a live claim rather than prose.

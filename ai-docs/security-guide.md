...
| Lost / stolen device | Phone with a week of local data and a valid signing key | **App-layer AES-256-GCM on the sensitive DB columns** (10-db §9.7 — sensitive VALUES are ciphertext; relational STRUCTURE/activity-shape is NOT, an accepted residual per D22), SecureStore keys, revocation. Captured media FILES on disk are **not** encrypted (task 158) — accepted until it lands |
...
The PIN authenticates a **user to the switcher on an already-enrolled device** for attribution. It is not the data-at-rest control (that is the app-layer column cipher, §6.4), not a server credential (that is the device token), and not strong. State this in code comments and owner-facing docs exactly as PRD-011 §8 does.

### 5.2 Verifier bundle — residual risk, stated honestly

Offline PIN verification (FR-1010) requires per-user verifiers **on the device**. They arrive exclusively via the control plane — the device bundle (api/02-auth §5.2) — as structured records `{ algorithm: 'argon2id', salt (16 random bytes, new on every set/change/reset), params record, hash (32 bytes), asOf }`, mirrored into the client `user_pin_verifiers` directory table, whose `salt`/`params`/`hash` columns are AEAD-encrypted at rest (§6.4; 10-db §9.7 rows 6-8). Two structural bounds hold by construction:
...
- Conclusion: **argon2id does not make a 6-digit PIN survive a dedicated offline attacker — it buys tens of minutes, not years.** Its jobs are: (a) making throttled on-device guessing slow, (b) per-user salts killing precomputation, (c) keeping the extracted-DB attack expensive enough to be deliberate rather than incidental. The controls that actually defend the bundle are the app-layer column cipher over `salt`/`params`/`hash` + SecureStore (extraction requires app/device compromise first, §6 — and note the lockout counters themselves are now plaintext, 03-state-machines §9), the escalating lockout (on-device guessing, api/02-auth §6.5), verifier minimization (a cracked DB exposes one store's users, not the tenant's), and blast-radius limits: a cracked PIN yields fraudulent *attribution on that one enrolled device* — every resulting op is still device-signed, logged, and the device is revocable.
...
- [ ] Verifier material lives ONLY in `user_pin_verifiers`, whose `salt`/`params`/`hash` are AEAD-encrypted at rest (§6.4), never in plaintext files or AsyncStorage. The surrounding row (`user_id`, `algo`, `as_of_*`) is plaintext by design — the §5.3 merge orders on it.
...
| SEC-AUTH-09 | verifier confidentiality | Verifier `salt`/`hash`/`params` are AEAD-encrypted at rest (§6.4; 10-db §9.7 rows 6-8) — scan app storage in the test harness for salt/verifier bytes. **FED, NOT CLOSED, by D22:** the Node lane proves those three columns are ciphertext through the real `writeVerifier` (`packages/harness/test/at-rest-column-encryption.test.ts`, falsified), but "the verifier bytes are ciphertext on a real device" is emulator-only and remains open — do not retire this row on the strength of the Node lane; no pushed op payload contains verifier material (scan payload bytes across a full pin change/reset cycle); comparison path is constant-time (statistical timing test on equal-length inputs) |
| SEC-AUTH-10 | on-device KDF benchmark recorded | The 2 GB-target benchmark harness runs argon2id at default params and asserts < 300 ms or triggers the documented floor decision — output committed as a build artifact, not folklore. **NOT discharged by D21** (owner ruling, 2026-07-22): D21 assumes the gate passes — which resolved D8's params (default kept, `api/02-auth` §5.3) — but an assumption produces **no artifact** and no measurement. Nothing has run; the id stays on `sec-pending-allowlist.json` (owner: task 27) until a real device produces the artifact. Retiring it against a test that merely pins the params is moving the yardstick (§2.11) |
...
Mechanism: api/02-auth (enrollment flow), PRD-011 §3.3/§7. Stack facts: expo-secure-store (values < 2 KB; encrypted-at-rest storage, **not** a non-extractable-key enclave), react-native-quick-crypto Ed25519 **and AES-256-GCM** (the at-rest column cipher, §6.4), op-sqlite `open({ name })` — **no `encryptionKey`** since D22.
...
- [ ] Signing key and the at-rest column-encryption key live in expo-secure-store, each value < 2 KB. **Qualify every claim:** SecureStore is Android-Keystore-encrypted-at-rest storage whose values are readable back by app code — it is NOT a hardware enclave and keys are extractable by anything running as the app. The spec, code comments, and owner-facing material must not say "hardware-backed" without this qualification.
...
### 6.4 Checklist — local at-rest encryption (column-cipher key lifecycle)

**D22 re-homed this control.** SQLCipher is GONE: op-sqlite's SQLCipher build vendored a second `libcrypto.so` that made the Android APK unassemblable (task 148), and every whole-file alternative was either forbidden (`pickFirst`), unshipped, or worse. At-rest confidentiality is now **application-layer AES-256-GCM over the signed-off sensitive columns** (10-db §9.7), using quick-crypto's already-linked OpenSSL — zero new native deps. **The key's origin, lifecycle and loss semantics are UNCHANGED; only what it encrypts changed.**

- [ ] Key = 32 CSPRNG bytes (quick-crypto), generated **once at enrollment**, stored in SecureStore, and fed to the **column cipher** — never derived from any PIN (PINs are per-user; the DB is per-device), never logged, never synced, never leaves the device. It is **no longer** an `open()` parameter; `open({ name })` takes no key.
- [ ] A key that is missing, empty, or not 32 bytes of hex **fails the open** — there is no path that proceeds with a weaker cipher or writes the sensitive columns in the clear.
...
- [ ] Key loss (SecureStore wipe, app data clear) = local data loss by design → re-enroll + re-pull from server. No key escrow, no recovery path — document as accepted. (Crypto-erase still works: deleting the key makes the protected VALUES unrecoverable.)
- [ ] **Claims must be scoped.** A DB file pulled off a device without SecureStore access yields **ciphertext for the 11 protected columns and readable structure for everything else**. Do NOT write "the database is encrypted" or "the file is not readable as SQLite" — since D22 the file *is* a readable SQLite file. The honest claim is the reshaped one, and the residual (metadata/activity-shape) is accepted.
- [ ] An encrypted column is never filtered/joined/ordered/indexed by content in SQL (10-db §9.7) — it would compare ciphertext and silently return a wrong answer.
- [ ] Any in-place plaintext→ciphertext conversion is followed by `VACUUM`, or the old cleartext survives in freed pages.
- [ ] Sealing is **unconditional** — the write path never inspects the value to decide whether it is "already encrypted". Shape-sniffing on write lets attacker-supplied content (a note body, a display name) steer itself into being stored in the clear, and there is no validation anywhere in this repo that stops a value from carrying the cipher marker.
- [ ] The cipher marker is **key-derived, not fixed**, so it cannot be forged. A fixed marker is steerable by anyone who can write ANY plaintext column — and the reachable instance was not a display-surface annoyance but a permanent one: `operations.agent_conversation_id` is plaintext by design, unbounded (`z.string().nullable()`) inside the SIGNED envelope, and read by the projection fold, so a marker-shaped value signed on a second enrolled device syncs store-wide and breaks `runRebuild` for that module **forever** — the op log is append-only, so the poisoned row cannot be removed without breaking the hash chain. Keys are per-device and never synced, which is what makes the derived marker unforgeable. Same fix covers the unvalidated `DeviceBundle` text (task 161). **Never make the marker a compile-time constant, and never make it per-connection random** (the latter orphans every previously-sealed row on the next launch).

**CEILING — the device AEAD binding is UNTESTED (call this out; do not let a green CI lane imply otherwise).** The codec is proven only against `node:crypto` (`nodeColumnAead`). The device binding `deviceColumnAead` (`apps/mobile/src/ports/aead.ts`) has exactly ONE consumer and ZERO tests, because quick-crypto is a JSI native module that cannot load under Node. If quick-crypto's GCM diverges from Node's in any framing detail (tag placement/length, IV handling, `update`/`final` chunking), **every sealed column on device becomes unreadable and nothing in CI would notice** — the same shape as the interop risk `CryptoPort` answers with the shared RFC vectors run on-device (08 §2.2 / task 27a). The equivalent here is owed: a device-lane test that seals with quick-crypto and opens with Node (and vice versa) over fixed vectors. Until it exists, "the columns are encrypted" is a NODE claim.
...
| SEC-DEV-06 | **The sensitive DB columns at rest are ciphertext** (scope RESHAPED by D22 — see §6.4) | **The claim changed with the mechanism.** The file IS a valid SQLite database now and opens without a key — that is no longer a finding. What must hold: copy the on-device DB file and grep its bytes → **none of the 11 protected values (10-db §9.7) appears in the clear**, each protected cell is a marked AEAD blob, and decrypting with a WRONG key **throws** (never garbage, never plaintext). Structure/ids/hashes ARE readable by design. **Status: the Node leg is SHIPPED and falsified** — `packages/harness/test/at-rest-column-encryption.test.ts` drives every column's real production writer against a file-backed DB and reads the raw bytes (plus `apps/mobile/src/media/queue.at-rest.test.ts` for `media_items.location`); breaking either write path was watched to go red. **The DEVICE leg is NOT closed:** "the raw file on a real Android device is ciphertext" needs the emulator and an assembled APK, and there is no Android SDK on this project's infra. Do not read the green Node lane as device-verified — it belongs with task 27's Android device-gates runner |
...
**⚠️ D22 CHANGED THIS FAILURE'S SHAPE — AND MADE IT QUIETER. The paragraph below describes the SQLCipher mechanism, which can no longer fire.** With no `open({encryptionKey})`, the restored plaintext file **opens fine** and `ensureDatabaseEncryptionKey()` mints a key, so neither `not_a_database` nor `missing_key` occurs: `bootWithLocalRecovery` never triggers, the app boots believing it is the old device (plaintext `meta_kv` still reads), and the failure surfaces as AEAD authentication errors deep in the UI instead of a loud boot brick that self-healed. **Net: a loud, self-healing failure became a silent half-enrolled one — strictly worse.** Fixing it needs a boot-time "can we decrypt a known cell?" probe with its own semantics for empty/partial DBs; that is deliberately NOT part of task 148 and is disclosed for its own task. Read the original text below as history:
...
- [ ] **The same guarantee holds for MUTATIONS, which the declared `storeId` alone cannot give you.** Two more rules are required and must both be present, or the bullet above is an overclaim: (1) a **store-scoped op TYPE carrying `storeId = null` is rejected** (05 §9.2a) — read from the module's declared `OperationDeclaration.scope`, never a hardcoded type list — because a null store bypasses a rule that only fires on a non-null store *and* widens pull scope to every device in the tenant; (2) every applier that UPDATEs/DELETEs an existing row **matches on `store_id` as well as `entityId`** (04 §4.1.3), because `entityId` is a global UUID and projection RLS is tenant-only, so entity-only matching is a cross-store write primitive. Audit each new module's mutation appliers against (2) — a create-only review misses this entirely.
...
| SEC-TENANT-06 | device→store write scope (D22) | Covers CREATE **and** MUTATE — the guarantee is "a device may write only its own store's ops", not "may only create". **Create:** a member device pushing an op scoped to another store of its tenant → per-op `SCOPE_VIOLATION`, op not logged, not folded, no poke to the foreign store, `device_anomalies` row recorded; the honest same-store sibling in the same batch still commits and is durably logged (§4.1). **Mutate (the `entityId` path):** a device signing `note_body_edited`/`note_archived` that NAMES a note living in another store must not change it, by either dodge — (a) `storeId = null`, which slips past a rule that only fires on a non-null store and is additionally pulled+re-folded by every device of the victim store (api/01-sync §4.1), and (b) `storeId` = the attacker's OWN store, which satisfies the equality rule trivially. Assert the victim row's `body`/`archived` are UNCHANGED and, for (a), that the op is not in the log at all. **Positive controls (all three mandatory):** the device's OWN store op is accepted and folded; a legitimate same-store edit **and** archive still fold (neither leg may close the hole by breaking the feature); and a legitimately tenant-scoped op (`storeId = null`, `platform.user_locale_changed`) is still accepted, so the rule is not "reject every null store". Order each attack op strictly AFTER the victim's create — on an equal timestamp the canonical key falls through to `deviceId` and the fold no-ops by accident, passing while the hole is open. Real PG16, driving production `createApp` + `serverOpRegistry` |

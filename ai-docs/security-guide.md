...
| SEC-SYNC-05 | oversized batch rejected | 501 ops, and separately > 1 MiB gzipped → `413`/`400` before any op is processed |
| SEC-SYNC-06 | malformed JSON after valid gzip | Valid gzip of invalid JSON → `400`; valid gzip of JSON failing the Zod push schema → `400` with schema error; no crash, no partial acceptance |

...
**Documented exceptions — exactly TWO in v0, enumerated below.** Both are places where a response distinguishes "exists but denied" from "does not exist", and both are allowed on the same reasoning: the caller can only ever probe with a value it *already holds*, so the response confirms a possession rather than enumerating anything. The list is **closed and mechanically pinned** — SEC-TENANT-04 parses this section, requires the harness's exception set to equal it exactly (a third exception documented here with no probe, or probed with no entry here, fails the sweep), and asserts that every *other* endpoint answers a cross-tenant id and a nonexistent one indistinguishably. Do not add a third by editing this paragraph: an exception is an owner ruling (CLAUDE.md §6), recorded in `decisions/` first.
...
**Documented exception 2 — push-token registration (`POST /v1/push/tokens`).** Owner ruling D22 §2 (task 141a). An Expo token already held by a device of **another tenant** returns `403 PERMISSION_DENIED` — fail-closed, because RLS hides the foreign row and ownership cannot transfer across tenants (api/04-push §2; task 118) — while a token nobody holds returns `200`. Held-vs-fresh is therefore distinguishable: a cross-tenant existence check on the token value. Justification — the **entropy argument**, the same shape as exception 1: an `ExponentPushToken[…]` carries ~88 bits of CSPRNG entropy, so a caller can only present a token it already holds; the `403` confirms a value already in its possession and never enumerates one, and a confusion set of 2^88 is not usefully reducible by probing (the 30/day per-device registration limit is charged *before* the collision check — `apps/server/src/routes/push.ts` — so the probe rate is capped on top of that). The alternative was weighed and rejected: making the two indistinguishable means either a lying `200` — the device then believes it is registered and silently receives no pushes, a correctness failure bounded by no entropy at all — or dropping the fail-closed check and breaching tenant isolation. **The exception covers the token value only.** The other `403` on this route (body `deviceId` ≠ the bearer's device) is not an existence oracle — it answers identically for a foreign device id and a nonexistent one, and SEC-TENANT-04's control leg asserts exactly that. SEC-TENANT-04's exception leg asserts the documented pair (`403` held / `200` fresh), so this paragraph is a live claim rather than prose.

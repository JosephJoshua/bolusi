...
- **Cross-tenant token collision → `403 PERMISSION_DENIED` (task 118; recorded by task 131 item 7).** `expo_push_token` carries a GLOBAL uniqueness constraint. If the submitted token is already held by a `push_tokens` row in **another** tenant, registration **fails closed** with `403 PERMISSION_DENIED` (rolling back the tx), because `bolusi_app` is `NOBYPASSRLS` and cannot see — let alone transfer — the foreign row without breaching tenant isolation (`routes/push.ts`, `isUniqueViolation`). This makes registration distinguish "this token exists in another tenant" (`403`) from "it does not" (`200`) — the **documented existence exception 2** of security-guide §2.2. The distinguishability is accepted there NOT on token secrecy but on the **probe budget**: §2's `30 registrations/day/device` cap makes the oracle uneconomical (D22 §2; SEC-TENANT-04 pins this pair). An Expo push token is not a caller-chosen value in practice, so a legitimate device never collides.
...
- Client triggers: (a) every app start, call `getExpoPushTokenAsync({ projectId })` and POST when the token differs from the last-registered value (token rotation, security-guide §9.1); (b) immediately after enrollment completes; (c) after a `DeviceNotRegistered` invalidation (§8) the next app start re-registers naturally via (a).
  - **V0 client narrowing of (a) (task 135 deliverable #1; recorded by task 156):** the client fires (a) only once a **session exists** (`Root` gates its effect on `sessionUserId !== null`), NOT on *every* app start. So a previously-enrolled device sitting on the switcher with nobody logged in does not (re)register until a login. This narrows (a) deliberately — §1 bounds the cost to a possibly-missed best-effort `sync` wake on a device where nobody logs in — and is NOT the same as the permissive server rule "`user_id` can be null" (that explains why the *server* tolerates a null acting user, not why the *client* may skip). The capability for strict (a) is already built (`transport.ts` omits `X-Acting-User` when the acting user is null); revisiting the narrowing is v1 scope, not a bug.

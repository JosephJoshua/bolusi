...
| `better-sqlite3` | *bootstrap-pin* | Node SQLite driver (built-in kysely `SqliteDialect`) backing **simulated devices** in the harness and the dual-dialect applier suite | Test-only; never imported by shipping packages. The shared `DbDriver` adapter over it lives in `@bolusi/sqlite-test-driver` (§3.3 rule 9). |
...
| `@bolusi/harness` | Node only | Chaos harness + N-device simulator: each simulated device = `@bolusi/core` + better-sqlite3-backed Kysely (via `@bolusi/sqlite-test-driver`) + noble crypto port + fetch transport, seeded via the `@bolusi/test-support` determinism kit + fakes; server side = `@bolusi/server` app in-process on PGlite or over HTTP to local dev server. Scenario semantics owned by the testing guide (testing-guide); this package is the machinery. Exit-criterion D4 runs here. |
| `@bolusi/sqlite-test-driver` | Node only (test-only) | The shared **better-sqlite3 → `DbDriver` adapter**: `createDriver(db)` + the value/row normalizers, plus `openMemoryDriver()` / `openFileDriver(path)`. Extracted from five byte-identical copies (2026-07-26 duplication audit; task 185 leg 4) so the ~80-line adapter has ONE home (CLAUDE.md §2.8). Value-imports `@bolusi/db-client` for `toDbError` + the `DbDriver` interface. Never imported by shipping source (§3.3 rules 6/9). |
...
| `ui` | `i18n` (key types only), `schemas` (**type-only** — canonical enum types like `SyncStatus`; hard rule 8), React Native, `expo-image`, `@expo/vector-icons` |
...
| `harness` | `core`, `modules` (manifest only), `schemas`, `test-support`, `@bolusi/server` (in-process, test-only), `sqlite-test-driver`, PGlite, noble |
| `sqlite-test-driver` | `db-client` (**value** — `toDbError` + the `DbDriver` interface it owns), better-sqlite3 |
...
8. The `@bolusi/ui` → `@bolusi/schemas` edge is **type-only** (task 193) — the sibling of rule 7. The design system reuses the canonical enum *types* from the registry (e.g. `SyncStatus`, which `SyncStatusChip` re-exports as `OperationSyncStatus`) instead of re-declaring a boundary-forced literal mirror that an out-of-package `enum-mirror-parity` arm then had to police (CLAUDE.md §2.8). ui stays **platform-free of zod**: `@bolusi/schemas`'s top-level `z.*` calls pull zod into any *value* importer, and `verbatimModuleSyntax` preserves what you write, so a value import would emit a runtime `require('@bolusi/schemas')` into ui's Hermes bundle. Only `import type` is allowed. **Enforced by `bolusi/boundaries` (`uiSchemasTypeOnly`)**, which rejects any non-`import type` of `@bolusi/schemas` from `packages/ui`, with a source-grep twin in `ui/test/package-hygiene.test.ts` behind it (the lint rule can be silenced per-file with an `eslint-disable`; the vitest twin cannot). `@bolusi/schemas` is a `dependency` of ui (not dev) because ui's shipped `.d.ts` re-exports the type, so downstream type resolution needs it resolvable — the same reason `@bolusi/i18n` is a dep "for key types only"; the edge carries no runtime, so neither the `require` nor zod ever reaches the bundle. Because the enum now crosses the boundary as a compiler-checked type — pinned exhaustively by a `Record<OperationSyncStatus, true>` in `SyncStatusChip.tsx`, which fails to compile on a missing *or* extra key — the parity arm that the hand-copied mirror needed was retired (its removal from `enum-mirror-parity` is part of task 193; that gate's full shrink is task 190).
9. `@bolusi/sqlite-test-driver` is the **single home** for the better-sqlite3 → `DbDriver` adapter (`createDriver` + the value/row normalizers), extracted from the five byte-identical copies the 2026-07-26 audit found (CLAUDE.md §2.8; task 185 leg 4). It is **test-only** (rule 6 extends to it: shipping source never imports it), and its edge to `db-client` is a **value** import — `toDbError` is a runtime helper and `DbDriver` is db-client's interface — so it is test-tooling like `harness` (whose db-client edge is also value), NOT like `test-support` (rule 7, type-only). `core`, `modules`, and `harness` consume `createDriver` / `openMemoryDriver` / `openFileDriver` from it and **no longer import better-sqlite3**. The direct better-sqlite3 edge now belongs to exactly **three** workspaces: `sqlite-test-driver` (the adapter), `db-client` (its keyed test opener **and** the codegen scratch DB, 10-db §11.4 — an independent use that keeps db-client an owner even after its test adapter routes through the package), and `apps/mobile` (its key-recording test opener, which wraps `createDriver` but keeps its own `new Database` to observe every key handed to the driver). Enforced by `bolusi/boundaries` (`DB_DRIVER_OWNERS`), whose entry list is this rule's three workspaces — the map and this prose must agree.
...
4. **op-sqlite bundled engines** recorded in §2.2: SQLite `3.51.3`, SQLCipher `4.14.0` (read from the installed 17.1.2 package's `cpp/` sources — the package vendors both regardless of whether we key with them). **The `package.json` `"op-sqlite"` config block drives only `performanceMode: true`; it does NOT drive `sqlcipher` — SQLCipher is OFF (D22/task 148), at-rest confidentiality is app-layer AES-256-GCM (10-db §9.7).** The Android build is where the block was exercised (the `libcrypto.so` collision that forced D22, deps row §2.2); the podspec/iOS discovery of the block is **unverified — iOS is not scaffolded** (task 85, D18 §5). *(Corrected 2026-07-28, task 131 item 8 / task 151 residual: the original record over-claimed a `sqlcipher`-driving mechanism the config no longer has, and an iOS podspec confirmation iOS-unscaffolded cannot give.)*

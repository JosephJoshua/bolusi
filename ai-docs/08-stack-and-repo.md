...
| Package manager | **pnpm 10** (workspaces + catalogs) | Exact patch recorded in root `packageManager` field at bootstrap; Corepack-activated. |
| TypeScript | latest stable 5.x at bootstrap, single hoisted version | Floor **5.4** (kysely 0.29.x minimum). One version for the whole repo. |

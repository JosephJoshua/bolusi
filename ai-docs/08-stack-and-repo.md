...
8. **Secret scanning**: gitleaks `8.30.1` — pre-commit hook (`.githooks/pre-commit`, installed via the root `prepare` script) + CI (unit job installs the binary for SEC-SECRET-02; `gitleaks/gitleaks-action@v2` scans history).

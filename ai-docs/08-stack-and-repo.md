...
9. **CI stages 7–8 deferred as named placeholders**: `ed25519-interop` (stage 7 → task 03) and `server-integration` (stage 8 → task 12) exist in `.github/workflows/ci.yml` as green no-op jobs echoing their owner — same pattern as the merge-gate placeholders (stages 9–12). They become real in their owning tasks; nothing may treat the placeholder echo as coverage. **Update (task 03):** stage 7 is now real (`pnpm test:ed25519-interop` — noble reproduces every `vectors/ed25519.json` vector); stage 6 is now real (record 6). **Update (task 32): stage 8 is now real** — `server-integration` runs `pnpm test:server` (22 files / 162 tests; falsified red-then-green). Three notes from the wiring, each load-bearing:

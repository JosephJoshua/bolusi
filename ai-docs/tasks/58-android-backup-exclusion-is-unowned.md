...
**RESIDUAL RISK — stated in the required words:** *the exclusion is present in the shipped manifest; that it behaves as documented on a real restore is unverified on-device.* There is no physical Android on this project (D12/D13); no Google Drive restore or device-to-device transfer was performed. **No green in this task may be read as a device-verified restore.** The on-device leg belongs with task 27's Android device-gates runner, alongside SEC-DEV-06. **This is the Android leg specifically** (SEC-DEV-08 is Android-scoped): under D17/D18 the iOS backup-exclusion leg is a *separate and larger* gap — no iOS runnable target of any kind exists here (task 80 §4), and iOS has no artifact-level exclusion mechanism at all (`security-guide` §6.6, task 84). The two gaps are not the same size.

...
**Tally: six rows across five tasks (SEC-OPLOG-02/05/09, SEC-AUTH-06, SEC-RT-02, SEC-AUTH-11) were mis-pointed — every single one found by the task that would have been blamed, and not one by the gate.** Six for six. A gate whose defect rate is 100% against its own purpose, discovered exclusively by the people it would have punished, is not a gate; it is a tax on the honest. That is the argument for ship-not-mention, and for the timing fix: none of these six would have fired until someone flipped a status to `done`, at which point the blast lands on whoever did the flipping.

...
1. **New inter-package edge — `test-support` → `@bolusi/db-client`, TYPE-ONLY. RATIFIED** (coordinator ruling, 2026-07-15) and recorded in 08 §3.3 (row + hard rule 7) on this branch. The suite must be typed against `DbDriver` (the ONE driver interface, CLAUDE.md §2.8 — duplicating the shape in test-support was the alternative and was rejected). No runtime edge exists: the emitted `dist/**/*.js` contains no db-client import; `verbatimModuleSyntax` + `consistent-type-imports` enforce it, and the driver handle is still injected by the runner.
...
3. **Client codegen uses `--camel-case` + `CamelCasePlugin`** — RESOLVED by coordinator ruling (2026-07-15) after this task raised it as an open question. 04 §2 requires appliers to be written once against `ProjectionDb` and run against both engines, and §11.3 pins the server to `--camel-case`; snake_case client types would make one applier unable to compile against both. Recorded normatively in **10-db §11.4** on this branch, then implemented: `--camel-case` in `scripts/codegen.ts`, `CamelCasePlugin` in the client Kysely, types regenerated. The flag and the plugin must change together — comments in both files say so.
   - Only the Kysely surface is camelized. `driver.execute` and the migration runner still speak the verbatim snake_case DDL of §9.
   - **Gotcha pinned by test:** `CamelCasePlugin` does not rewrite identifiers *inside* a raw ``sql`` `` fragment, but it does camelize the RESULT keys (``sql`PRAGMA foreign_keys` `` → `{ foreignKeys: 1 }`).
   - Identifier identity is verified as far as it currently can be: server §8 and client §9.6 declare **byte-identical column names** for the shared `notes` table, and camelization is a pure function of the column name, so both sides yield `tenantId`/`storeId`/`createdBy`/`lastEditedAt`/`editCount`. A file-vs-file diff is not yet possible — the server's generated types land with **task 05**, which should confirm the match on arrival.

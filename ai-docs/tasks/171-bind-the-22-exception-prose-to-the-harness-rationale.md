...
> **DONE 2026-07-28.** Bound each `DOCUMENTED_EXISTENCE_EXCEPTIONS` entry to its §2.2 source by a CITATION that resolves (the task's stated achievable close — "assert the LINK resolves", NOT a prose-equality gate the reviewer warned would rot). Added `docExceptionIndex` to `ExistenceException` (route-walker.ts); each entry sets it (media=1, push=2). A new SEC-TENANT-04 test resolves each entry's `(docExceptionIndex, endpoint)` pair against `parseDocumentedExistenceExceptions(SECURITY_GUIDE)` and fails if it is not one §2.2 enumerates, AND pins that the printed `rationale`'s own "§2.2 exception <n>" text equals `docExceptionIndex` (human citation ↔ machine citation cannot drift). Blind-parse control (T-14b): the resolver asserts §2.2 parsed to a non-empty set first, and a denominator that it resolved EVERY entry. **FALSIFIED (§2.11) both directions:** (A) set the push entry's `docExceptionIndex` to 1 → RED "cites §2.2 exception 1, but §2.2 enumerates 1→GET /v1/media/:id, 2→POST /v1/push/tokens — the citation does not resolve"; (B) SWAPPED §2.2's exception numbers (media↔push) with the harness untouched → RED naming the pairing drift (this is the task's falsify #1: a §2.2 change the harness did not follow); restored → 11/11 green. **Honest scope:** this closes a rationale citing the WRONG exception, or a §2.2 renumbering/re-pairing the harness did not follow. It does NOT mechanically gate a free-prose justification edit inside the paragraph body (the 141a "88 bits entropy" text) — the task's own deliverable rules that out ("do not assert the two prose texts are semantically equal … would rot into a string-equality check somebody deletes"). The current rationale prose is already correct (30/day budget, NOT entropy); the citation binding stops the two copies drifting on the checkable axis.

...
- **Reproduce the value first** (T-11), and this is the deliverable that proves the task was worth doing: take task 46's int8 bug (or an equivalent silent-marshalling defect), and show it **GREEN on the PGlite lane and RED on the new lane**. That single pair is the whole argument. If it does not reproduce, **STOP and report** — the premise moved (four premises have been refuted on this project; every refusal was correct).

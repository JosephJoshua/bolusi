...
CHAOS mapping (harness scenarios exercising this surface end-to-end in task 26 — this task must not make them impossible): CHAOS-02, CHAOS-12, plus the client halves of CHAOS-03, CHAOS-05 (T3/T5/T7 client behavior), CHAOS-06(b)(c).

**SEC ids owed by THIS surface:** SEC-OPLOG-09, **and the CLIENT leg of SEC-SYNC-02** — security-guide §4.2 requires "ops pushed in the same window → `DEVICE_REVOKED`, **kept client-side as `rejected`**", which is the acceptance bullet above (per-op `rejected` marking + surfacing, never silent); task 16 shipped only that id's SERVER legs (401 + `DEVICE_REVOKED`) and deliberately keeps the id OUT of its test titles so SEC-META-01 does not read the id as fully shipped. Ship this leg under a title embedding `SEC-SYNC-02` verbatim and remove its row from `sec-pending-allowlist.json`. The remaining SEC-SYNC-01..10 rows are server-side (task 16).

...
> **DONE 2026-08-05 (both legs).** **Leg A** needed NO code — task 155's per-keystroke write-through already preserves the draft before any navigation (D23 §1 "preserve" done by construction), so the push tap's existing `setRoute` navigates immediately without destroying anything and without a ConfirmSheet (D23: retention, not a prompt). **Leg B** is the fix: `App.tsx`'s deep-link effect now guards `!switchingRef.current` (a value ref, so the effect stays keyed on the TAP alone — adding `switching` to its deps would re-fire the stale `pushRoute` on a later switcher open/close, a phantom navigation), so a tap landing mid-switch no longer folds into the switcher's `origin` — the same drift `openSyncStatus` guards (task 145). **Tests** (`test/live-shell-push.test.tsx`, over the real Root + push seam): Leg A — a DIRTY editor + conflict tap navigates to sync-status with NO `notes.editor.discard` sheet; Leg B — switcher open + tap → the route does not move, and a hardware back returns to home (where the switch opened), not the pushed route. **§2.11-falsified:** removing the `!switchingRef` guard reds Leg B (back lands on sync-status, origin drifted). mobile suite + lint + typecheck green. **DEFERRED (the two LOW folds + coverage gaps below — NOT the ruling/legs, safe to leave):** the duplicated discard gate in `NoteEditor` (requestLeave vs the effect body), the hardware-back-while-sheet-open destination rewrite, and the sync-chip/clean-language-chip coverage gaps — small, independent follow-ups.

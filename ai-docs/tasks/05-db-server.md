...
> - **Config lives per package, not shared.** 08 §3.3 hard rule 2 forbids db-client and db-server importing each other, so `packages/db-server/src/camel-case.ts` is *this* package's config and db-client grows its own. This is not a §2.8 violation: §2.8 governs **logic**, and the thing that prevents the bug is the guard — which cannot be shared anyway, since each side must run against its own catalog, its own generated file, its own engine. The single source of truth for the *value* is the doc: 10-db §11.3 (server) and §11.4 (client) both state the option normatively, both packages implement it, both guards enforce it against their own live schema. That is stronger than one shared constant, which can be wrong in one place with nothing checking.
...
- **SEC-TENANT-02**: as `bolusi_app` with `app.tenant_id = A`: SELECT on B's rows → 0 rows; INSERT with `tenant_id = B` → error (WITH CHECK); UPDATE/DELETE targeting B's rows → 0 affected. Plus fail-closed: a transaction with NO `set_config` **raises an error** — it does not read zero rows. 10-db §6.3 owns this ("error → fail closed") and the one-arg `current_setting` is deliberate: the two-arg form returns NULL and degrades to a *silent* zero-rows, which is a fail-open dressed as a fail-closed. **Both error codes are reachable and must be pinned**, because which one you get is connection-state-dependent: `42704` on a connection that has never set the GUC (the placeholder does not exist), `22P02` on one that set it transaction-locally earlier (the GUC reverts to `''`, and `''::uuid` fails). `22P02` is the **pooled production shape**. Pinning both is correct, not accidental flakiness. (Corrected 2026-07-15: this line previously said "reads zero rows", contradicting the owning doc — the acceptance was wrong, not the implementation.)

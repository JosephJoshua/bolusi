...
| 116 | react-native-web + Playwright visual harness so the screens render to pixels in a headless browser and can be screenshotted/interaction-tested (the layer the test-renderer cannot cover); binds in-memory fakes for native modules via the platform-free-core ports (owner request) | todo | 96 |

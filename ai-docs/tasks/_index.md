...
| 15 | sync-client (loop, triggers, backoff, SyncState, staleness, quarantine) | in-progress | 06, 10 |
...
| 39 | `DB` is `any` for every consumer of @bolusi/db-server — all of apps/server untyped against the schema (from task 07) | in-progress | 05 |

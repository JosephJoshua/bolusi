...
| 91 | **HIGH** iOS restore-to-new-hardware permanently BRICKS the app: restored DB + non-restored THIS_DEVICE_ONLY key → wrong-key open → `boot()` renders nothing forever, no recovery. iOS-triggered (backup asymmetry, task 84), platform-neutral fix (catch not_a_database→wipe+re-enrol). Median device event for a repair franchise (from impl-ios) | in-progress | — |

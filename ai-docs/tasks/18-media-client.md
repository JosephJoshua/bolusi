...
## CORRECTIONS (orchestrator, 2026-07-16) — two dead premises in this file, both verified

**1. The `media_items` migration is NOT this task's to ship — it already ships.** `packages/db-client/src/migrations/001-initial-schema.ts:89-114` (`MEDIA_QUEUE`: the table + `idx_media_items_queue`, byte-for-byte per 10-db §9.4), registered in `CLIENT_MIGRATIONS` at `:263`, `MediaItems` already in `src/generated/db.ts`, and `idx_media_items_queue`'s `WHERE upload_status <> 'uploaded'` already asserted in `test/migrations.test.ts`. Task 04 shipped it. **The global migration-serialization constraint on this task is therefore void.** Found by impl-18 tracing to the producer instead of taking the brief (T-16).

**2. `pendingMediaCount` is NOT this task's to write — it already ships and is live.** `packages/core/src/sync/state.ts` defines it (formula per 06 §4, incl. the orphan exclusion), `core/src/sync/loop.ts:263` **calls** it, exported at `sync/index.ts:51`, tested at `test/sync/state-and-devices.test.ts:198`. Task 15 shipped it. Writing a second violates §2.8. It living in `sync/` is correct — 06 §4 says it is *"recomputed by the sync loop (api/01-sync §6)"*. The "no import path from the media engine into op-sync internals" rule concerns the **other** direction and remains satisfiable.

**3. `mediaRefSchema`'s home: ruled `@bolusi/schemas`, not `@bolusi/core`** — see the task filed for the `06 §3.2` spec fix. Core structurally cannot author Zod (`08 §3.3:159` grants core `schemas`/`canonicalize`/`kysely` types only; zod is a **devDependency**; `core/src/module/strict-schema.ts:6` states it outright). `zMediaRef` needs `zUuidV7`/`zSha256Hex`/`zMsEpoch`/`zLocation`, all of which live in schemas — and `zLocation` reuse is **mandatory**, because `tooling/eslint/src/index.js:17-18` allowlists float props to `packages/schemas/src/envelope.ts` alone.

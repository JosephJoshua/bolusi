...
| 9 | PinAuth (PIN lockout) | derived per (userId, deviceId) from `pin_attempt_state` | `unlocked` `delayed` `locked_out` | client DB, SQLCipher-encrypted (`pin_attempt_state` — shape owned by api/02-auth §6.5, DDL 10-db §9.5) | client runtime (machine owned by api/02-auth §6.5; §9 mirrors it) |
...
Per **(userId, deviceId)** pair, enforced entirely locally (offline is the normal case). State persists in the SQLCipher-encrypted client DB table `pin_attempt_state` — `{userId, deviceId, consecutiveFailures, windowStartedAt, notBefore}` (shape: api/02-auth §6.5; DDL: 10-db §9.5) — surviving app restart (SEC-AUTH-03). Clearing it requires wiping app data, which also destroys the device enrollment and signing key, so bypassing lockout costs the attacker the enrollment. Other users on a shared terminal are never blocked by one user's failures.

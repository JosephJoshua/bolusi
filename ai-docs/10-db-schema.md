...
| Server types | `kysely-codegen` **0.20.0** with `--camel-case` against the migrated dev DB; output committed to `packages/db-server/src/generated/db.ts`. **A `.ts` file, not `.d.ts` — for the same reason as the client row below** (§11.4): `tsc` does not copy an input `.d.ts` into `dist/`, so `dist/index.d.ts`'s `export type { DB } from './generated/db.js'` dangles, TypeScript resolves it to `any`, and every consumer silently gets an untyped `DB` while `tsc` stays green. Runtime uses **`new CamelCasePlugin({ underscoreBetweenUppercaseLetters: true })`** so TS names are camelCase over the snake_case DDL below — the option is REQUIRED, not a preference (§11.3). CI re-runs codegen and fails on diff. |
| Client DB | `@op-engineering/op-sqlite` **17.1.2**, package.json flags `{"op-sqlite": {"performanceMode": true}}` — **`sqlcipher` is deliberately OFF** (D22/task 148: its OpenSSL collided with quick-crypto's and `:app:mergeReleaseNativeLibs` refused two `libcrypto.so`; at-rest confidentiality moved to app-layer column encryption, §9.7). EAS dev builds (never Expo Go). **Single connection app-wide** (op-sqlite hard rule). All access behind a thin wrapper in `packages/db-client` (`@bolusi/db-client`) so expo-sqlite remains a swap target. |
...
| Client types | same kysely-codegen 0.20.0, **`--camel-case`** (§11.4), run in CI against a scratch SQLite file built by applying the client migrations via `better-sqlite3` (dev-only dep); output committed to `packages/db-client/src/generated/db.ts`. A `.ts` file, not `.d.ts`: `tsc` does not copy an input `.d.ts` into `dist/`, so the built package's type references would dangle. Runtime uses `CamelCasePlugin({ underscoreBetweenUppercaseLetters: true })` — see §11.4 for why the option is mandatory. |
...
## 9. Client SQLite DDL (op-sqlite 17.1.2, application-layer column encryption)

Open sequence (in `packages/db-client`, the single wrapper): `open({name: 'bolusi.db'})` — **no `encryptionKey`.** D22 dropped SQLCipher (its OpenSSL collided with quick-crypto's and made the Android APK unassemblable — task 148), so the SQLite **file is plaintext** and at-rest confidentiality is provided by **application-layer AES-256-GCM over the sensitive columns** (§9.7). The 32-byte key is unchanged in origin and lifecycle — random, generated at first launch, stored in `expo-secure-store` (< 2 KB; encrypted-at-rest storage, **not** a hardware enclave — qualify all claims) — it now feeds that column cipher instead of `open()`. Then pragmas: `journal_mode = WAL`, `foreign_keys = ON`, `busy_timeout = 5000`, `synchronous = NORMAL`. One connection for the whole app; concurrency comes from WAL, never from extra connections. The device's Ed25519 private key and device token also live in `expo-secure-store` — **never** in SQLite. Bulk paths (pull apply, rebuild) use prepared statements + `executeBatch`.

### 9.7 At-rest column encryption (D22 — the reshaped guarantee)

**Sensitive VALUES are ciphertext; relational STRUCTURE is plaintext.** A forensic reader of the raw file on a non-running device learns the activity *shape* (how many ops of what type, against which entities/users/devices, when) but not the sensitive values. That residual is accepted (D22); it is strictly weaker than SQLCipher's whole-file opacity and **must not be described as "the database is encrypted"**.

**The encrypted set is exhaustive and signed off (D22 addendum 2). Encrypt exactly these, and no others:**

| # | Column | Why |
| - | ------ | --- |
| 1 | `operations.payload` | business data |
| 2 | `operations.signed_core_jcs` | contains the payload verbatim |
| 3 | `operations.location` | GPS |
| 4 | `notes.title` | business data |
| 5 | `notes.body` | business data |
| 6 | `user_pin_verifiers.salt` | PIN verifier material (SEC-AUTH-09) |
| 7 | `user_pin_verifiers.hash` | PIN verifier material |
| 8 | `user_pin_verifiers.params` | PIN verifier material |
| 9 | `media_items.location` | capture GPS |
| 10 | `quarantined_ops.signed_core_jcs` | a held-aside op is no less sensitive |
| 11 | `users_directory.name` | employee PII |

**Plaintext, and REQUIRED to stay so:** all ids/FKs, `entity_type`/`type`/`schema_version`, `seq`/`arrival_seq`/`timestamp_ms`, `hash`/`previous_hash`/`signature` (a signature is not secret), `sync_status`/`synced_at`, every enum, counters, `media_sha256` (a hash), `signing_key_public` (a PUBLIC key), `meta_kv.value` (device ids/scalars — secrets live in SecureStore), `locale`, denial `reason`/`target`/`surface`, conflict keys, and `media_items.local_path`.

**The rule that makes this safe:** an encrypted column can **never** be filtered, joined, ordered or indexed by content in SQL — it would compare ciphertext. Before encrypting any column, confirm no `WHERE`/`JOIN`/`ORDER BY`/index touches it. No index in §9 does. The one known query that did was the switcher's `ORDER BY name`; its sort now happens in JS after decryption (`listSwitcherUsers`). Adding a column to this set is a new decision, not an implementation detail.

**Format:** each value is stored as `<marker> ‖ base64(nonce(12) ‖ ciphertext ‖ tag(16))` in the same `TEXT` column, with a **fresh random 96-bit nonce per value**. The marker is `<scheme prefix> ‖ base64(first 9 bytes of HMAC-SHA256(markerSubkey, label)) ‖ ':'` (72 bits, 12 base64 chars), where `markerSubkey` is itself HMAC-derived from the DB key so the AES-256 key is never used directly as an HMAC key — **key-derived, and that is a security property, not a detail.** Read dispatch is value-based (it must be: raw `sql` and `SELECT *` hide which column a value came from, and keying it to column NAMES would silently hand ciphertext to any query that aliased a column). A *fixed* marker therefore let anyone steer the read seam: `operations.agent_conversation_id` is plaintext by design, rides the signed envelope as an unbounded `z.string().nullable()`, and is read by the projection fold — so a second enrolled device could sign an op whose conversation id merely *looked* sealed and permanently break `runRebuild` for that module on every device in the store, in an append-only table whose row cannot be deleted without breaking the hash chain. Deriving the marker from the per-device key (never synced) makes it unforgeable by anyone who does not hold that device's key. **The derivation must stay a pure function of the key** — a per-connection random tag would pass every same-session test and then orphan every previously-sealed row on the next launch. Columns keep their declared types, so the committed codegen types are unaffected — this is a value transform, not a schema change (§11).

**Mechanism:** encryption is transparent at the connection layer. Query-builder writes (`operations` via the op-store, `notes` via module appliers) are sealed by a Kysely plugin, so **an applier never learns its columns are encrypted**; the `<DB>`-generic raw-`sql` writers (pull `operations`, verifiers, directory, quarantine, media) seal at the value through a cipher registered on the connection. Decryption is uniform on every read. Migration 3 establishes this and **`VACUUM`s**: converting a column in place leaves the old plaintext in freed pages until the file is rewritten, which is a real leak.
...
3. Run `kysely-ctl migrate:latest` against the dev DB → run kysely-codegen 0.20.0 (`--camel-case`) → commit generated types. CI regenerates and diffs. The server runtime wires `CamelCasePlugin` from `packages/db-server/src/camel-case.ts` — **it MUST carry `{ underscoreBetweenUppercaseLetters: true }`; see step 4 for why the defaults are wrong for this schema.**
4. Client: add the embedded migration; CI builds a scratch SQLite DB from all client migrations, runs kysely-codegen against it **with `--camel-case` (as the server does in step 3)**, diffs committed types. The client runtime wires Kysely's `CamelCasePlugin` alongside the custom dialect (08 §2.2), so generated identifiers and query-builder identifiers agree. **Both engines therefore expose identical camelCase identifiers over the same snake_case DDL** — this is what lets a module's appliers be written once against `ProjectionDb` and run against client SQLite *and* server Postgres/PGlite alike (04 §2). Raw SQL and the driver-level helpers are unaffected: they speak the snake_case column names in this doc verbatim.
   - **`CamelCasePlugin` MUST be constructed with `{ underscoreBetweenUppercaseLetters: true }` on BOTH sides — the default options are wrong for this schema.** kysely-codegen's camelizer and the plugin's inverse are **not inverses at defaults**: codegen turns the column `op_a_id` into the property `opAId`, but default `snakeCase('opAId')` is `'op_aid'`, a column that does not exist. Such a query *typechecks* and fails at runtime with "no such column". The trigger is a single-letter segment between camel humps — live today in `conflicts.op_a_id` / `op_b_id` (§9.6). Both sides therefore keep one shared config object (`packages/db-client/src/connection.ts` `CLIENT_CAMEL_CASE_OPTIONS`; `packages/db-server/src/camel-case.ts`) — separate copies only because 08 §3.3 hard rule 2 forbids db-client and db-server importing each other; **this doc line is the shared source of truth.**
   - **Generated types are `.ts` on BOTH sides, and that is load-bearing, not stylistic.** `tsc` does not copy an input `.d.ts` into `outDir`, so a `src/generated/db.d.ts` produces no `dist/generated/` at all — while `dist/index.d.ts` still re-exports from it. The re-export dangles, TypeScript resolves the missing module to **`any`**, and every consumer of the package typechecks against `any` forever, with a green `tsc` and no diagnostic anywhere. The server shipped exactly this defect (task 39): `DB` was `any` in all of `apps/server`, so no table or column reference in the app was ever checked. A `.ts` input is compiled and emitted, so the types actually leave the package. **The rule is "we never export a type we do not emit"** — it binds any future generated output in any package, not just these two.
   - **`apps/server/test/db-types-reach-consumers.test.ts` is the guard for the above** and must keep running from *outside* `@bolusi/db-server` — checking from inside the package cannot see this bug (the source types are correct; only the built artifact is missing). It asserts `DB` is not `any`, that a known table resolves, and that a bogus table key is rejected. It fails **red** when the emit regresses.
   - **Each side ships a property-by-property guard test** (`test/codegen-camel-case.test.ts` on both) that re-derives the column for EVERY generated property and asserts it exists in the live schema. Type names alone are identical and correct on both engines even when the mapping is broken, so comparing generated types — or spot-checking a column — cannot catch this class of defect. Nothing but the inverse check against the real schema counts.
   - **At-rest column encryption does NOT change generated types (§9.7).** The encrypted columns stay `TEXT` and hold base64 ciphertext; the cipher is a connection-layer *value* transform, so `db:codegen` output is byte-identical before and after. If a codegen diff ever appears alongside an encryption change, something changed the SCHEMA and needs a spec edit here first — the diff is the signal, not noise to regenerate away.
5. Never edit generated type files by hand; never define a table interface manually. **Reformatting counts as editing:** every generated output MUST be listed in `.prettierignore`. Otherwise the pre-commit formatter rewrites the committed file, the next codegen run "drifts" back to raw output, and the CI codegen-diff gate (§11.3) becomes permanently unsatisfiable — a gate that can never go green trains everyone to ignore it.
...
| Client at-rest column-encryption key | `expo-secure-store`, random 32 bytes hex, generated at first launch. Feeds the app-layer AES-256-GCM column cipher (§9.7) — **not** an `open()` parameter since D22 |

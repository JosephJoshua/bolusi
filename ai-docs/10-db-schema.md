...
**Format:** each value is stored as `<marker> ‖ base64(nonce(12) ‖ ciphertext ‖ tag(16))` in the same `TEXT` column, with a **fresh random 96-bit nonce per value**. Columns keep their declared types, so the committed codegen types are unaffected — this is a value transform, not a schema change (§11).

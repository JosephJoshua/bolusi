...
> **Update 2026-07-22 — D21 answers the *decisions*, not the *measurements*.** The owner ruled: proceed as if 27b's performance gates pass (`decisions/2026-07-22-assume-device-performance-passes.md`). So the two rows below that read "undecided/unvalidated" are now **decided by assumption** — D8 keeps the default `m=32768/t=3/p=1`, D6's throughput rationale is ratified — each **assumed to pass per D21 (owner ruling, 2026-07-22); unverified on device**. **No number was produced.** The recommendation at the end of this section ("a single device-farm run converts all three from 'assumed' to 'measured'") is therefore **still live and still the only thing that would**: a device farm remains the cheapest way to turn these assumptions into measurements, and the trigger below is unchanged. What D21 removed is the *blocking* of v0 on them, not the gap.

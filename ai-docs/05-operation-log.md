...
6. **A `payload.mediaRef` is bound to the envelope signer (SEC-MEDIA-08; task 140 Leg B).** A `mediaRef` is self-describing — it carries its OWN `userId`/`deviceId`, which in v0 duplicate the envelope because capture and attach happen in one command (06-media-pipeline §3.2). If a *structurally-valid* ref's `deviceId` (or `userId`) is a string that is **not** the envelope's, the op is rejected `SCOPE_VIOLATION` — exactly as rule 1 binds op.`deviceId` to the token device. Without it a device signs a note whose ref names another device's `mediaId`, and every puller renders that device's photo as this note's evidence (06 §6; the server half of the composed attack task 140 closed — the client half is SEC-MEDIA-07). This is generic over op TYPE (any op carrying `payload.mediaRef`), runs BEFORE the schema step (so a malformed ref stays `SCHEMA_INVALID`, never green-for-the-wrong-reason), and a null `mediaRef` (no photo, §3) has nothing to bind and passes. **v0-scope, revisit-at-v1:** the `deviceId` half is correct for "capture+attach in one command"; a v1 attach-previously-captured-media flow (06 §3.2) must relax it deliberately, not inherit it by accident.

...
2. op.`tenantId` == device.`tenantId`; op.`storeId` is null (tenant-scoped) or belongs to the tenant. **A device may write only its OWN store's ops (D22, SEC-TENANT-06):** a non-null op.`storeId` that is a store of the tenant *other than the pushing device's `store_id`* is rejected `SCOPE_VIOLATION` — closing the gap where a device at store A could write an op into store B of the same tenant (a mechanic recording a repair note in another branch's book). Tenant-scoped ops (op.`storeId` = null) are **not** store-bound and pass: a member device legitimately emits `platform.user_locale_changed` (tenant-scoped — the preference follows the user to every device, 01-domain-model §6) and a tenant-scoped-entity `platform.conflict_acknowledged`. Member devices always carry a `store_id` (10-db-schema §4 `CHECK (kind = 'system' OR store_id IS NOT NULL)`) and the runtime stamps it into every store-scoped op (02-permissions §5.2), so for those op.`storeId` == device.`store_id`. The tenant **system** device (`store_id` null) signs only server-built `platform.conflict_detected`, which is chained server-side (10-db-schema §3) and **never pushed** (01-domain-model §3.6 — "no carve-outs to §9's scope checks"); so this store rule is keyed on the device's own store rather than its kind, needs no system-device carve-out, and is an ADDITIONAL, narrower scope on top of RLS (10-db-schema §6), never a replacement.

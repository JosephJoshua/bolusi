...
**Encryption at rest (SEC-MEDIA-09, task 158).** Captured media files under `<documentDirectory>/media/` and the render cache under `<cacheDirectory>/media/` are **AES-256-GCM ciphertext on disk** — the DB column encryption (task 148) protects the `media_items` row, but the photo the `local_path` points at is the actual evidence and must not sit as a plaintext JPEG a forensic reader of a lost device can lift. The on-disk format is a **STREAM construction**: a random per-file nonce **base(8)** header, then **256 KiB frames** each `ciphertext ‖ tag(16)` (no stored per-frame nonce — the frame plaintext size is pinned equal to the api/03-media §4 wire chunk). Each frame's nonce is **derived** as `base ‖ BE32(frameIndex + finalFlag)`, so the file is bound as an ordered whole: reordering frames, truncating whole frames, duplicating a frame, or splicing a frame in from another media file all decrypt under the wrong derived nonce and **fail the GCM tag** (not just a within-frame bit-flip). A plaintext chunk offset still maps to exactly one on-disk frame, so the streamed hash (§2.2 step 6, decrypted frame-by-frame into an incremental digest) and the drain's per-chunk read (§5.5) decrypt one frame at a time — peak memory one frame, never the whole file (the render path is the one exception: it buffers the whole plaintext to write the temp, bounded by the ≤ 300 KiB v0 photo cap). The key is a **sibling of the SecureStore DB key** (`deriveMediaFileKey`, domain-separated HMAC — no new secret; key separation from the column cipher); the random base prevents `(key, nonce)` reuse across files. The signed `sha256`/`sizeBytes` are always over the **plaintext**. Two bounded plaintext windows remain, both in OS-evictable cache: the **pre-move capture scratch** (`<cacheDirectory>/media-capture/`, deleted by the encrypting cache→document move, §2.2 step 5) and the **transient render temp** (`<cacheDirectory>/media-render/`, decrypted just long enough for `<Image>` to decode, re-derivable) — **the render temps are cleared on every pruning pass** so a lost device holds no plaintext copies of recently-viewed media. The encrypt/decrypt logic (incl. structural tamper) is proven in the Node lane (`packages/core`); that expo-file-system writes these bytes on a device is emulator-deferred (D21). **v1 tripwire:** the whole-plaintext render-temp write is bounded by the ≤ 300 KiB photo cap — v1 video (the 10 MiB api/03-media §3.1 cap) must stream the render decrypt to the temp instead of buffering.

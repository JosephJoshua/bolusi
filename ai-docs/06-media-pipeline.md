...
**Encryption at rest (SEC-MEDIA-09, task 158).** Captured media files under `<documentDirectory>/media/` and the render cache under `<cacheDirectory>/media/` are **AES-256-GCM ciphertext on disk** — the DB column encryption (task 148) protects the `media_items` row, but the photo the `local_path` points at is the actual evidence and must not sit as a plaintext JPEG a forensic reader of a lost device can lift. The on-disk format is **256 KiB framed** (`nonce(12) ‖ ciphertext ‖ tag(16)` per frame, the frame plaintext size pinned equal to the api/03-media §4 wire chunk), so a plaintext chunk offset maps to exactly one on-disk frame and the streamed hash (§2.2 step 6) and the drain's per-chunk read (§5.5) decrypt one frame at a time — peak memory one frame, never the whole file. The key is a **sibling of the SecureStore DB key** (`deriveMediaFileKey`, domain-separated HMAC — no new secret; key separation from the column cipher). The signed `sha256`/`sizeBytes` are always over the **plaintext**; a wrong key or tampered byte fails LOUD (never silent garbage). Two bounded plaintext windows remain, both in OS-evictable cache: the **pre-move capture scratch** (`<cacheDirectory>/media-capture/`, deleted by the encrypting cache→document move, §2.2 step 5) and the **transient render temp** (`<cacheDirectory>/media-render/`, decrypted just long enough for `<Image>` to decode, re-derivable). The encrypt/decrypt logic is proven in the Node lane (`packages/core`); that expo-file-system writes these bytes on a device is emulator-deferred (D21). **v1 tripwire:** the whole-plaintext render-temp write is bounded by the ≤ 300 KiB photo cap — v1 video (the 10 MiB api/03-media §3.1 cap) must stream the render decrypt to the temp instead of buffering.

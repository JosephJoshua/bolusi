...
Screens never render a raw `FlatList` and never `.map()` rows. `List` owns two things structurally:
...
2. **The four §5 states as a discriminated union** (`loading | empty | error | unauthorized | ready`). A screen that forgets `unauthorized` fails to COMPILE rather than failing review. §5 says a screen missing any of the four fails review; this makes the build the first gate.
...
- **Lists:** consumed ONLY through the §3.13 `List` primitive — screens never touch `FlatList` directly and never `.map()` rows. `List` wraps RN `FlatList` (no `@shopify/flash-list` dep in v0 — see §3.13 for the verified engine comparison and the pre-vetted swap target) with fixed-height rows + `getItemLayout`, `windowSize` ≈ 7, `initialNumToRender` ≈ 10, `removeClippedSubviews` on Android. Cursor pagination via query `nextCursor` (04-module-contract §6) with `onEndReached`.

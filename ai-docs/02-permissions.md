...
6. **PIN-command targeting (offline commands):** changing a PIN via permission `auth.pin_change` may target only the acting user; an owner PIN reset (permission `auth.user_reset_pin`) and a lockout clear (permission `auth.pin_unlock`) may target only users present in the device's `users_directory`. Violations deny `DomainError('PERMISSION_DENIED')`, reason `restriction_violated`, **and are audited**: the handler declares the denial and the enforcement point emits an `auth.permission_denied` op (reason `restriction_violated`) through the runtime's one denial emitter (§7) — the audit trail must be complete exactly where a privileged-target attack is most serious (FR-1045). Command surfaces are owned by api/02-auth §6.3; the forged-op path is closed by the §4 push-validation exception.
...
| Emitted by | The permission runtime itself, at the single enforcement point (§4), for **both** evaluator denials (a permission the caller lacks) **and** handler-declared **restriction** denials (§5.4 targeting/privileged-target rules). A §5.4 restriction is decided inside a handler because it needs the directory, so it cannot be the evaluator's answer at step 2: the handler *declares* the denial and the enforcement point *emits* it — through the **same** throttled emitter, so `auth.permission_denied` stays exactly one emission path and one of the five runtime-emitted types (§4). Bypasses the command layer (it IS the command layer). Never recursive: its emission is never permission-checked. |
...
**The op-log denial trail above is the CLIENT arm** — it covers denials decided by the command/query runtime on-device (§4). It does **not** cover, and no reader should assume it covers, the **SERVER arm**: the identity control-plane endpoints (api/02-auth §5.4) enforce this registry server-side and deny over **HTTP `403 PERMISSION_DENIED`**, never through the runtime, so their denials cannot be an `auth.permission_denied` op (that envelope is store-scoped and device-signed; a control session has neither device nor store). A server denial where the **authenticated actor lacks the required authority** — the §4.5 permission gate (`requirePermission`) and the handler-declared §5.4 restrictions (the tenant-grant rule §5.4.2 and the privileged-target rule §6.6) — is therefore recorded in **`identity_audit`** (action `permission.denied`, reuse of the closed `DenialReason` set above), the same server-side sink that already audits the directory mutation the denied caller was attempting. This closes FR-1045 on the server without a third mechanism (§2.8): the two denial trails (`auth_permission_denials` for the client, `identity_audit`.`permission.denied` for the server) are disjoint by construction and complete together. Deliberately **excluded** from the server trail, because they are not "actor lacks a permission": FR-1036 not-found masquerades (`403` for a store/role the caller may not even see — no permission id to log), a wrong-credential-channel `403` (a device token on a control-session-only endpoint), and op-push scope/binding rejections (audited as `device_anomalies`, security-guide §3.1).
...
**Shared-device honesty (do not oversell):** sync pull is *device*-scoped (api/01-sync §4.1) — the local op log on a shared device contains payloads for the whole store, regardless of which user is active. Query-layer gating protects every legitimate read surface (UI, future agent, future API); it does not protect against forensic extraction from the device database. At-rest protection is **application-layer AES-256-GCM on the sensitive columns** (D22; 10-db §9.7) with the key in expo-secure-store — which is encrypted-at-rest storage, **not** a non-extractable-key enclave; a rooted device with the app's keys is a breach, per PRD-011 §8. **This is weaker than it used to read:** op-log *payloads*, note bodies and GPS are ciphertext, but the op log's STRUCTURE — which users acted on which entities, of what type, when — is plaintext on a shared device's file. Forensic extraction therefore yields the store's activity shape even though it does not yield the values. True "never on this device" for high-sensitivity fields (e.g. v1 `inventory.view_cost_price`) requires **per-role/per-user sync scope** — deferred with OQ-1103, see roadmap.md. Any v1 module introducing a field whose exposure to co-workers on a shared device is unacceptable MUST raise that dependency, not rely on this section alone.

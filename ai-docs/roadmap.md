...
| R22 | Store switcher + multi-store active-store context (FR-1034) | v1 | Rides with R15 — non-functional until cross-store pull scope lands | v0 rule (pinned): `ctx.storeId` = the enrolled device's store, ALWAYS (02-permissions §5.2; 04-module-contract §5.2). **Label copy parked here** (task 30): `Ganti Toko` / `Switch Store`. It is deliberately NOT in the v0 label catalog — it was `auth.switchStore`, which violates the 07-i18n §3.1 key grammar (2 segments), and its legal `<namespace>.<screen-or-area>.<label>` name depends on the screen the switcher lands on. Whoever builds R22 adds the row to the `auth` catalog with the real screen segment |
...
| R28 | Authorship binding for legacy (v1/v2) media-ref render (task 153 Part B) | **closed by the version floor in v0** — reopens only at T10 | A producer of a pre-v3 `note_created` ref can exist (see T10) | The render arm `loadLocalMediaOnly` (`apps/mobile/src/media/remote-cache.ts`) returns `{kind:'local'}` **unverified** for a legacy attachment — `ThumbnailRef.legacy` carries no `sha256` (type-enforced) and `notes/applier.ts` keeps `mediaSha256` null — an evidence-substitution shape identical to the v3 defect task 140 fixed. **It is unreachable in v0's lineage:** v0 is the first release and ships at `NOTE_CREATED_SCHEMA_VERSION = 3` (05-operation-log §2), so no producer of a v1/v2 ref has ever existed — a legacy ref cannot be authored, only a *future* schemaVersion downgrade or a migration back-filling legacy refs could create one (T10). Closing it needs the note's **author device** at the render path (a `10-db-schema` projection column + a `06-media-pipeline` port change) — deferred to whenever T10 trips, not built speculatively. SEC-MEDIA-07 (task 153 Part A) guards the *live* v3 client arm today |
...
| T10 | A pre-v3 `note_created` ref becomes producible — `NOTE_CREATED_SCHEMA_VERSION` drops below 3, a migration back-fills legacy (`ThumbnailRef.legacy`) refs, or an older client's ops are ingested | R28's "closed by the version floor" reasoning rests on there being NO producer of a v1/v2 media ref in v0's lineage; any of these creates one, and the unverified legacy render arm (`loadLocalMediaOnly`) becomes a live evidence-substitution vector — the exact shape task 140 fixed for v3 | Reopen R28: implement the author-device authorship binding (projection column + port change) BEFORE the producing change ships; do not rely on SEC-MEDIA-07, which guards only the v3 signed-hash arm |

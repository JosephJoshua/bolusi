...
| R22 | Store switcher + multi-store active-store context (FR-1034) | v1 | Rides with R15 — non-functional until cross-store pull scope lands | v0 rule (pinned): `ctx.storeId` = the enrolled device's store, ALWAYS (02-permissions §5.2; 04-module-contract §5.2). **Label copy parked here** (task 30): `Ganti Toko` / `Switch Store`. It is deliberately NOT in the ui-labels v0 seed — it was `auth.switchStore`, which violates the 07-i18n §3.1 key grammar (2 segments), and its legal `<namespace>.<screen-or-area>.<label>` name depends on the screen the switcher lands on. Whoever builds R22 adds the row to ui-labels.md with the real screen segment |

...
**Accepted risk, closer named:** everything between task 03 and task 27 builds on "JCS bytes match on device". If they do **not** match on 0.17, the discovery invalidates every signature produced in between. Task **27b** closes it — RN 0.86's real Hermes is 0.17 — **but only if 27b runs THESE exact vectors, not a subset.** That requirement is now written into 27b's acceptance. Until then the honest phrasing is "unproven on the shipping VM", not "solved". Alternatives rejected: building Hermes 0.17 from source in CI (heavy, brittle); the Android-emulator fallback (an emulator's Hermes is real 0.17 — 27a should run these vectors there too, which converts this from "unproven" to "proven on emulator Hermes 0.17", a real upgrade over 0.13).

...
> **Forward note (2026-07-22, D21) — the PERFORMANCE half is now ASSUMED, not owed; nothing here was measured.** Owner ruling **D21** (`decisions/2026-07-22-assume-device-performance-passes.md`) directs us to proceed as if 27b's Part C gates pass. So of the three unproven claims tabled below: **argon2id p95** and **op-sqlite write throughput** are **assumed to pass per D21 (owner ruling, 2026-07-22); unverified on device** — which *decides* D8 (default kept) and *ratifies* D6, both by assumption rather than by measurement. **SQLCipher at-rest is untouched** by D21: it is a *correctness* claim and belongs to 27a's emulator lane. Read this doc's "consequence" cells as **what a real device could still refute**, not as open decisions — and read nothing in it as a number: no device figure exists for any row. The physical-device facts are unchanged (there is still no device, and no emulator figure is a device figure); D21 records that the owner **accepts** the residual risk, not that it went away. v0's exit no longer waits on 27b for the performance half; it still waits on 27a (SEC-AUTH-09 leg 1) and task 28 — both CI events, not hardware.

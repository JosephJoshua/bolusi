...
| **T-14** | **A guard must not be able to check nothing.** Any sweep, enumeration, or parse-driven assertion SHALL assert its own coverage: expected item count, non-zero properties per parsed unit, an aggregate floor set below today's real number. A loop over an empty collection passes silently — the failure mode of a guard must never be "verified nothing, reported green". |

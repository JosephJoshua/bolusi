...
const config: ExpoConfig = {
  ...
  plugins: [
    ...
    // ── The on-device harness entry activity (task 27a/175) ───────────────────────────────────────
    // `apps/mobile/android/` is generated by prebuild and git-ignored, so `HarnessActivity` (the
    // component scripts/harness-device.mjs launches) can only reach the manifest through a config
    // plugin. It is a NO-OP unless `EXPO_PUBLIC_BOLUSI_TEST_HARNESS=1` at prebuild, so the exported
    // activity never lands in a production/preview build. The native `Log.i` emitter it relies on is the
    // local Expo module `modules/harness-native`, whose native source (build.gradle + Kotlin `Module`) is
    // COMMITTED under `modules/harness-native/android/` so autolinking compiles it into the APK — a bare
    // `android/` gitignore had silently dropped it (task 175 review). See withHarnessActivity.ts.
    //
    // Referenced by STRING PATH, not a nested import: Expo's plugin resolver transpiles a `.ts` plugin,
    // but its config loader does NOT transpile a `.ts` imported into app.config.ts (require resolves
    // only `.js`). The plugin's exports are still typechecked + unit-tested via test/harness-activity-plugin.
    './plugins/withHarnessActivity',
  ],
};
